...
def task_2(number_of_networks : int):
    training_losses=[]
    training_accuracies=[]
    validation_losses=[]
   ...
   # Define transformations and datasets
    ...
    optimizer = [torch.optim.SGD(model_voter.parameters(), lr=0.002, momentum=0.9) for model_voter in model.models]
    ...
    for epoch in range(EPOCHS_LIMIT):
        ...
        for i, model_voter in enumerate(model.models):
            ...
            for x, y in iter(train_loader):
                optimizer[i].zero_grad()# notice that by default, the gradients are accumulated, hence we need to set them to zero
    ...
    torch.save(best_model, "models_task_2_ensemble_of_networks.pt")
    ...
    save_loss_accuracy_plot(
        ...
        training_accuracies, 
        validation_accuracies,  
        "loss_and_accuracy_task_2_ensemble_of_networks.png"
    )

    save_confusion_matrix(
        confusion_matrix(all_labels, all_predictions),
        test_accuracy,
        "confusion_matrix_task_2_ensemble_of_networks.png",
        test_dataset
    )
